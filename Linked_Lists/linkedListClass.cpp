...
struct linkedListClass::node* linkedListClass::initNode(string s, int i) {
...
void linkedListClass::addNode(struct node* newNode) {
...
void linkedListClass::insertNode(struct node* newnode) {
...
struct linkedListClass::node* linkedListClass::searchName(struct node* ptr, string name) {
    while(name != ptr->name) {
        ptr = ptr->next;
        if(ptr == NULL) {
            break;
...
struct linkedListClass::node* linkedListClass::searchId(struct node* ptr, int id) {
    while(id != ptr->id) {
        ptr = ptr->next;
        if(ptr == NULL) {
            break;
...
void linkedListClass::deleteNode(struct linkedListClass::node* ptr) {
    struct node* temp, *prev;
...
void linkedListClass::deleteList(struct node* ptr) {
    struct node* temp;

    if(head == NULL) {
        return;    // don't try to delete an empty list
    }
...
void linkedListClass::displayNode(struct linkedListClass::node* ptr) const {
    cout << ptr->id << ": " << ptr->name << endl;
}

void linkedListClass::displayList(struct linkedListClass::node* ptr) const {
    if(!ptr) {
